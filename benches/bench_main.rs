...
fn bench_decompress_offset15_micro(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("Decompress offset15 Micro");
    group.throughput(Throughput::Bytes(size as u64));

    group.bench_with_input("libdeflate-rs offset15 micro", &size, |b, &_size| {
...
fn bench_decompress_offset29_micro(c: &mut Criterion) {
    let size = 1024 * 1024; // 1MB
    // 29 bytes pattern
    let pattern = b"ABCDEFGHIJKLMNOPQRSTUVWXYZ012";
    let mut original_data = Vec::with_capacity(size);
    while original_data.len() < size {
        original_data.extend_from_slice(pattern);
    }
    ...
    let mut group = c.benchmark_group("Decompress offset29 Micro");
    group.throughput(Throughput::Bytes(size as u64));

    group.bench_with_input("libdeflate-rs offset29 micro", &size, |b, &_size| {
        let mut decompressor = Decompressor::new();
        b.iter(|| {
            decompressor
                .decompress_deflate_into(&compressed_data[..compressed_size], &mut out_buf)
...
fn bench_decompress_offset35_micro(c: &mut Criterion) {
    let size = 1024 * 1024; // 1MB
    // 35 bytes pattern
    let pattern = b"ABCDEFGHIJKLMNOPQRSTUVWXYZ012345678";
    let mut original_data = Vec::with_capacity(size);
    while original_data.len() < size {
        original_data.extend_from_slice(pattern);
    }
    ...
    let mut group = c.benchmark_group("Decompress offset35 Micro");
    group.throughput(Throughput::Bytes(size as u64));

    group.bench_with_input("libdeflate-rs offset35 micro", &size, |b, &_size| {
        let mut decompressor = Decompressor::new();
        b.iter(|| {
            decompressor
                .decompress_deflate_into(&compressed_data[..compressed_size], &mut out_buf)
...
fn bench_decompress_offset34_micro(c: &mut Criterion) {
    let size = 1024 * 1024; // 1MB
    // 34 bytes pattern
    let pattern = b"ABCDEFGHIJKLMNOPQRSTUVWXYZ01234567";
    let mut original_data = Vec::with_capacity(size);
    while original_data.len() < size {
        original_data.extend_from_slice(pattern);
    }
    ...
    let mut group = c.benchmark_group("Decompress offset34 Micro");
    group.throughput(Throughput::Bytes(size as u64));

    group.bench_with_input("libdeflate-rs offset34 micro", &size, |b, &_size| {
        let mut decompressor = Decompressor::new();
        b.iter(|| {
            decompressor
                .decompress_deflate_into(&compressed_data[..compressed_size], &mut out_buf)
...
fn bench_decompress_offset38_micro(c: &mut Criterion) {
    let size = 1024 * 1024; // 1MB
    // 38 bytes pattern
    let pattern = b"ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789ab";
    let mut original_data = Vec::with_capacity(size);
    while original_data.len() < size {
        original_data.extend_from_slice(pattern);
    }
    ...
    let mut group = c.benchmark_group("Decompress offset38 Micro");
    group.throughput(Throughput::Bytes(size as u64));

    group.bench_with_input("libdeflate-rs offset38 micro", &size, |b, &_size| {
        let mut decompressor = Decompressor::new();
        b.iter(|| {
            decompressor
                .decompress_deflate_into(&compressed_data[..compressed_size], &mut out_buf)
...
fn bench_decompress_offset42_micro(c: &mut Criterion) {
    let size = 1024 * 1024; // 1MB
    // 42 bytes pattern
    let pattern = b"ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789abcdef";
    let mut original_data = Vec::with_capacity(size);
    while original_data.len() < size {
        original_data.extend_from_slice(pattern);
    }
    ...
    let mut group = c.benchmark_group("Decompress offset42 Micro");
    group.throughput(Throughput::Bytes(size as u64));

    group.bench_with_input("libdeflate-rs offset42 micro", &size, |b, &_size| {
        let mut decompressor = Decompressor::new();
        b.iter(|| {
            decompressor
                .decompress_deflate_into(&compressed_data[..compressed_size], &mut out_buf)
...
fn bench_decompress_offset22_micro(c: &mut Criterion) {
    let size = 1024 * 1024; // 1MB
    // 22 bytes pattern
    let pattern = b"ABCDEFGHIJKLMNOPQRSTUV";
    let mut original_data = Vec::with_capacity(size);
    while original_data.len() < size {
        original_data.extend_from_slice(pattern);
    }
    ...
    let mut group = c.benchmark_group("Decompress offset22 Micro");
    group.throughput(Throughput::Bytes(size as u64));

    group.bench_with_input("libdeflate-rs offset22 micro", &size, |b, &_size| {
        let mut decompressor = Decompressor::new();
        b.iter(|| {
            decompressor
                .decompress_deflate_into(&compressed_data[..compressed_size], &mut out_buf)
...
fn bench_decompress_offset52_micro(c: &mut Criterion) {
    let size = 1024 * 1024; // 1MB
    // 52 bytes pattern
    let pattern = b"ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789abcdefghijklmnop";
    let mut original_data = Vec::with_capacity(size);
    while original_data.len() < size {
        original_data.extend_from_slice(pattern);
    }
    ...
    let mut group = c.benchmark_group("Decompress offset52 Micro");
    group.throughput(Throughput::Bytes(size as u64));

    group.bench_with_input("libdeflate-rs offset52 micro", &size, |b, &_size| {
        let mut decompressor = Decompressor::new();
        b.iter(|| {
            decompressor
                .decompress_deflate_into(&compressed_data[..compressed_size], &mut out_buf)
...
fn bench_decompress_offset60_micro(c: &mut Criterion) {
    let size = 1024 * 1024; // 1MB
    // 60 bytes pattern
    let pattern = b"ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789abcdefghijklmnopqrstuvwx";
    let mut original_data = Vec::with_capacity(size);
    while original_data.len() < size {
        original_data.extend_from_slice(pattern);
    }
    ...
    let mut group = c.benchmark_group("Decompress offset60 Micro");
    group.throughput(Throughput::Bytes(size as u64));

    group.bench_with_input("libdeflate-rs offset60 micro", &size, |b, &_size| {
        let mut decompressor = Decompressor::new();
        b.iter(|| {
            decompressor
                .decompress_deflate_into(&compressed_data[..compressed_size], &mut out_buf)
...
fn bench_decompress_offset36_micro(c: &mut Criterion) {
    let size = 1024 * 1024; // 1MB
    // 36 bytes pattern
    ...
    let mut group = c.benchmark_group("Decompress offset36 Micro");
    group.throughput(Throughput::Bytes(size as u64));

    group.bench_with_input("libdeflate-rs offset36 micro", &size, |b, &_size| {
        let mut decompressor = Decompressor::new();
        b.iter(|| {
            decompressor
                .decompress_deflate_into(&compressed_data[..compressed_size], &mut out_buf)
...
fn bench_decompress_offset44_micro(c: &mut Criterion) {
    let size = 1024 * 1024; // 1MB
    // 44 bytes pattern
    let pattern = b"ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789abcdefgh";
    let mut original_data = Vec::with_capacity(size);
    while original_data.len() < size {
        original_data.extend_from_slice(pattern);
    }
    ...
    let mut group = c.benchmark_group("Decompress offset44 Micro");
    group.throughput(Throughput::Bytes(size as u64));

    group.bench_with_input("libdeflate-rs offset44 micro", &size, |b, &_size| {
        let mut decompressor = Decompressor::new();
        b.iter(|| {
            decompressor
                .decompress_deflate_into(&compressed_data[..compressed_size], &mut out_buf)
...
fn bench_decompress_offset3_micro(c: &mut Criterion) {
    let size = 1024 * 1024; // 1MB
    // 3 bytes pattern
    let pattern = b"123";
    let mut original_data = Vec::with_capacity(size);
    ...
    let mut group = c.benchmark_group("Decompress offset3 Micro");
    group.throughput(Throughput::Bytes(size as u64));

    group.bench_with_input("libdeflate-rs offset3 micro", &size, |b, &_size| {
        let mut decompressor = Decompressor::new();
        b.iter(|| {
            decompressor
                .decompress_deflate_into(&compressed_data[..compressed_size], &mut out_buf)
...
fn bench_decompress_offset46_micro(c: &mut Criterion) {
    let size = 1024 * 1024; // 1MB
    // 46 bytes pattern
    let pattern = b"ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789abcdefghij";
    let mut original_data = Vec::with_capacity(size);
    while original_data.len() < size {
        original_data.extend_from_slice(pattern);
    }
    ...
    let mut group = c.benchmark_group("Decompress offset46 Micro");
    group.throughput(Throughput::Bytes(size as u64));

    group.bench_with_input("libdeflate-rs offset46 micro", &size, |b, &_size| {
        let mut decompressor = Decompressor::new();
        b.iter(|| {
            decompressor
                .decompress_deflate_into(&compressed_data[..compressed_size], &mut out_buf)
...
fn bench_decompress_offset54_micro(c: &mut Criterion) {
    let size = 1024 * 1024; // 1MB
    // 54 bytes pattern
    let pattern = b"ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789abcdefghijklmnopqr";
    let mut original_data = Vec::with_capacity(size);
    while original_data.len() < size {
        original_data.extend_from_slice(pattern);
    }
    ...
    let mut group = c.benchmark_group("Decompress offset54 Micro");
    group.throughput(Throughput::Bytes(size as u64));

    group.bench_with_input("libdeflate-rs offset54 micro", &size, |b, &_size| {
        let mut decompressor = Decompressor::new();
        b.iter(|| {
            decompressor
                .decompress_deflate_into(&compressed_data[..compressed_size], &mut out_buf)
...
fn bench_decompress_offset62_micro(c: &mut Criterion) {
    let size = 1024 * 1024; // 1MB
    // 62 bytes pattern
    ...
    let mut group = c.benchmark_group("Decompress offset62 Micro");
    group.throughput(Throughput::Bytes(size as u64));

    group.bench_with_input("libdeflate-rs offset62 micro", &size, |b, &_size| {
        let mut decompressor = Decompressor::new();
        b.iter(|| {
            decompressor
                .decompress_deflate_into(&compressed_data[..compressed_size], &mut out_buf)
...
criterion_group!(
    ...
    bench_decompress_offset3_micro,
    ...
    bench_decompress_offset29_micro,
    ...
    bench_decompress_offset36_micro,
    bench_decompress_offset35_micro,
    bench_decompress_offset44_micro,
    bench_decompress_offset8_micro,
    bench_decompress_offset60_micro,
    bench_decompress_offset52_micro,
    bench_decompress_offset22_micro,
    bench_decompress_offset58_micro,
    bench_decompress_offset42_micro,
    bench_decompress_offset38_micro,
    bench_decompress_offset50_micro,
    bench_decompress_offset34_micro,
    bench_decompress_offset46_micro,
    bench_decompress_offset54_micro,
...
fn bench_decompress_offset58_micro(c: &mut Criterion) {
    let size = 1024 * 1024; // 1MB
    // 58 bytes pattern
    let pattern = b"ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789abcdefghijklmnopqrstuv";
    let mut original_data = Vec::with_capacity(size);
    while original_data.len() < size {
        original_data.extend_from_slice(pattern);
    }
    ...
    let mut group = c.benchmark_group("Decompress offset58 Micro");
    group.throughput(Throughput::Bytes(size as u64));

    group.bench_with_input("libdeflate-rs offset58 micro", &size, |b, &_size| {
        let mut decompressor = Decompressor::new();
        b.iter(|| {
            decompressor
                .decompress_deflate_into(&compressed_data[..compressed_size], &mut out_buf)
...
fn bench_decompress_offset33_micro(c: &mut Criterion) {
    let size = 1024 * 1024; // 1MB
    // 33 bytes pattern
    let pattern = b"ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456";
    let mut original_data = Vec::with_capacity(size);
    while original_data.len() < size {
        original_data.extend_from_slice(pattern);
    }
    ...
    let mut group = c.benchmark_group("Decompress offset33 Micro");
    group.throughput(Throughput::Bytes(size as u64));

    group.bench_with_input("libdeflate-rs offset33 micro", &size, |b, &_size| {
        let mut decompressor = Decompressor::new();
        b.iter(|| {
            decompressor
                .decompress_deflate_into(&compressed_data[..compressed_size], &mut out_buf)
...
fn bench_decompress_offset50_micro(c: &mut Criterion) {
    let size = 1024 * 1024; // 1MB
    // 50 bytes pattern
    let pattern = b"ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789abcdefghijklmn";
    let mut original_data = Vec::with_capacity(size);
    while original_data.len() < size {
        original_data.extend_from_slice(pattern);
    }
    ...
    let mut group = c.benchmark_group("Decompress offset50 Micro");
    group.throughput(Throughput::Bytes(size as u64));

    group.bench_with_input("libdeflate-rs offset50 micro", &size, |b, &_size| {
        let mut decompressor = Decompressor::new();
        b.iter(|| {
            decompressor
                .decompress_deflate_into(&compressed_data[..compressed_size], &mut out_buf)
