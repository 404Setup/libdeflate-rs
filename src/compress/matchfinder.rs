...
impl MatchFinder {
    ...
    where
        F: FnMut(usize, usize),
    {
        if pos.saturating_add(3) > data.len() {
            return (0, 0);
        }
...
mod tests {
    ...
    #[test]
    fn test_overflow_repro() {
        let mut mf = MatchFinder::new();
        let data = b"abc";
        let pos = usize::MAX - 2;
        // This should not crash or cause UB
        let (len, offset) = mf.find_match(data, pos, 10);
        assert_eq!((len, offset), (0, 0));
    }
}
