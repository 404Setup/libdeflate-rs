            ...
            unsafe {
                ...
                if offset >= length {
                    ...
                    if offset == 1 || offset == 2 || offset == 4 {
                        ...
                        if copied + 8 <= length {
                            std::ptr::write_unaligned(dest_ptr.add(copied) as *mut u64, pat1);
                            copied += 8;
                        }
                        if copied + 8 <= length {
                            std::ptr::write_unaligned(dest_ptr.add(copied) as *mut u64, pat2);
                            copied += 8;
                        }
                        if copied + 8 <= length {
                            std::ptr::write_unaligned(dest_ptr.add(copied) as *mut u64, pat3);
                            copied += 8;
                        }
                        while copied < length {
                        ...
                        if copied + 8 <= length {
                            std::ptr::write_unaligned(dest_ptr.add(copied) as *mut u64, pat4);
                            copied += 8;
                        }
                        if copied + 8 <= length {
                            std::ptr::write_unaligned(dest_ptr.add(copied) as *mut u64, pat5);
                            copied += 8;
                        }
                        while copied < length {
                            *dest_ptr.add(copied) = *src_ptr.add(copied);
                    ...
                    if offset < 8 {
                        if offset == 1 || offset == 2 || offset == 4 {
                            ...
                            while i + 40 <= length {
                                ...
                                (out_next.add(i + 8) as *mut u64).write_unaligned(pat1);
                                (out_next.add(i + 16) as *mut u64).write_unaligned(pat2);
                                (out_next.add(i + 24) as *mut u64).write_unaligned(pat3);
                            ...
                            while i + 56 <= length {
                                ...
                                (out_next.add(i + 24) as *mut u64).write_unaligned(pat3);
                                (out_next.add(i + 32) as *mut u64).write_unaligned(pat4);
                                (out_next.add(i + 40) as *mut u64).write_unaligned(pat5);
